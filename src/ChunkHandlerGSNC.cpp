...
{
	using namespace std;
	cChunkHandlerGSNC::cChunkHandlerGSNC()
	  : m_pCallbackGSNCHandler(nullptr)
	{
	}

	void cChunkHandlerGSNC::registerCallbackHandler(cCallbackGSNC *pHandler)
	{
		m_pCallbackGSNCHandler = pHandler;
	}

	bool cChunkHandlerGSNC::processChunk(const vector<char> &vchData, bool bBigEndian)
	{
		// check that there is enough data for an even number of shorts
		if(((vchData.size()) & 0x3) != 0)
			return false;
		if(m_pCallbackGSNCHandler == nullptr)
			return true;
		...
		m_pCallbackGSNCHandler->callbackGSNC(lTimestamp_ns, vsIqData);
		return true;
	}

	void cChunkHandlerGSNC::writeChunkGSNC(cPXGWriterBase &stream, int64_t lTimestamp_ns, const vector<short> &vsIqData) const
	{
	...
	void cChunkHandlerGSNC::repack(const cPackingGIQP &oInputPacking, const cPackingGIQP &oOutputPacking, const vector<short> &vsInputIQData, vector<short> &vsOutputIQData)
	{
