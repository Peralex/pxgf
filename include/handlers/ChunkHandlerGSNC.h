#ifndef CHUNKHANDLERGSNC_H
#define CHUNKHANDLERGSNC_H
...
namespace pxgf
{
	...
	class cChunkHandlerGSNC : public cChunkHandler
	{
	public:
		...
		{
		public:
			/// Called on the callback handler whenever a GSNC
			/// chunk is processed.
			...
			virtual void callbackGSNC(int64_t lTimestamp_ns, const std::vector<short> &vsIqData) = 0;
		};
		...
		void registerCallbackHandler(cCallbackGSNC *pHandler);
		...
		void writeChunkGSNC(cPXGWriterBase &stream, int64_t lTimestamp_ns, const std::vector<short> &vsIqData) const;
	...
	private:
		cCallbackGSNC *m_pCallbackGSNCHandler; ///< Object which implements the callback handler interface.
	};
}
